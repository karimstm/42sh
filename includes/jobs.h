...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/02/29 14:08:00 by amoutik           #+#    #+#             */
/*   Updated: 2020/02/29 14:13:41 by amoutik          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					process_push(t_list_process *list,
							pid_t pid, t_node *node);
void					init_process_list(t_list_process *p);
void					job_push(t_job_list *jobs, t_list_process *p,
						pid_t pgid);
void					init_job_list(t_job_list *jobs);
void					job_notification(t_job_list *jobs);
int						mark_process_status(t_job_list *jobs,
...
void					foreground_job(t_job_list *job_list,
						t_job *job, int cont);
void					background_job(t_job *job, int cont);
void					dummy_process(t_job_list *job_list,
						t_node *node, t_job_kind kind);
void					job_waiting(t_job_list *job_list, t_job *job);
void					ft_print_node(t_node *node);
const char				*ft_strsignal(int sig);
void					continue_job(t_job_list *job_list,
							t_job *j, int foreground);
int						ft_fg(char **args);
int						ft_bg(char **args);
