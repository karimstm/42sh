...
t_list_simple_command	*do_globing(char *node)
{
	glob_t					glob_holder;
	...
	if (glob_holder.gl_pathv)
		while (glob_holder.gl_pathv[++i])
			token_push(glob_lst, ft_strdup(glob_holder.gl_pathv[i]), TOKEN_WORD);
	else
		token_push(glob_lst, ft_strdup(node), TOKEN_WORD);
	return (glob_lst);
}
...
void	init_globing(t_list_simple_command *list)
{
	...
	{
		next = current->next;
		if ((res = do_globing(current->name))->node_count)
		{
			if (prev)
				prev->next = res->head;
			else
