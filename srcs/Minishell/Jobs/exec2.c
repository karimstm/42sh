...
/*   Updated: 2020/03/02 21:41:41 by amoutik          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				setup_redirection(t_process *p, int doexit)
{
	if (p->node->redir)
	{
		...
		p->node->redir = reverse_redirection(p->node->redir);
		if (execute_redirection(p->node->redir))
		{
			if (doexit)
				exit(EXIT_FAILURE);
			return (0);
...
int				run_built_in(t_blt_line *blt_line, t_process *process)
{
		...
		&& *cmds)
	{
		...
		ft_free_strtab(cmds);
		JOB_LIST->status = process->status;
		process->completed = 1;
